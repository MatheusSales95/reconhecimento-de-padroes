...
metodologicamente rigorosa ($F_1 = 0{,}411$ em generalização cross-cena),
...
de carbono. No Brasil, a elevada frequência de queimadas em todos os biomas reforça a necessidade de métodos escaláveis e reprodutíveis que
...
\begin{table}[!t]
  ...
  \begin{tabular}{lll}
    \toprule
    \textbf{Atributo} & \textbf{O que mede} & \textbf{Contribuição} \\
    \midrule
...
%=====================================================================
\section{Objetivo Geral e Configuração dos Modelos}

Todos os modelos deste trabalho têm o mesmo objetivo central: \textbf{classificação
binária pixel a pixel} de imagens Landsat~8/9, determinando se cada pixel pertence
a uma área que sofreu queimada recente. A saída de cada modelo supervisionado é
um mapa georreferenciado com:
\begin{itemize}
  \item \textbf{Classe~1} — Pixel queimado;
  \item \textbf{Classe~0} — Pixel não queimado.
\end{itemize}

Os atributos de entrada são derivados das bandas espectrais B4 (vermelho),
B5 (infravermelho próximo) e B7 (infravermelho de ondas curtas), bem como dos
índices NDVI e NBR, calculados para os períodos pré e pós-queimada. O modelo
SOM, único não supervisionado, utiliza também as variações $\Delta$NDVI e
$\Delta$NBR, e produz três classes: Queimada, Não Queimada e Água.

A Tabela~\ref{tab:modelos_io} resume a configuração de entradas e saídas de
cada modelo.

\begin{table}[!t]
  \caption{Entradas e saídas por modelo}
  \label{tab:modelos_io}
  \centering
  \resizebox{\columnwidth}{!}{%
  \begin{tabular}{llll}
    \toprule
    \textbf{Modelo} & \textbf{Atributos} & \textbf{Qtd.} & \textbf{Saída} \\
    \midrule
    KNN           & B4, B5, B7, NDVI, NBR (pré/pós) & 10 & Classe 0/1 \\
    SVM Linear    & NDVI pré e pós                   & 2  & Classe 0/1 \\
    SVM RBF       & B4, B5, B7, NDVI, NBR (pré/pós) & 10 & Classe 0/1 + probabilidade \\
    MLP           & B4, B5, B7, NDVI, NBR (pré/pós) & 10 & Probabilidade via Softmax \\
    SOM           & B4, B5, B7, NDVI, NBR, $\Delta$NDVI, $\Delta$NBR & 7 & Q / Não-Q / Água \\
    Random Forest & B4, B5, B7, NDVI, NBR (pré/pós) & 10 & Classe 0/1 \\
    \bottomrule
  \end{tabular}}
\end{table}

Os modelos supervisionados (KNN, SVM Linear, SVM RBF, MLP e Random Forest)
foram treinados sobre máscaras de referência geradas a partir do limiar
$\Delta$NBR $> 0{,}65$, com as features $\Delta$NBR e $\Delta$NDVI
\textbf{excluídas} do conjunto de atributos para evitar vazamento de dados
(\textit{feature leakage}). O SOM opera sem rótulos.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.6\columnwidth]{imgs_knn/cena_treinamento.png}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.6\columnwidth]{imgs_knn/cena_teste.png}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.6\columnwidth]{imgs_knn/cena_teste_classificada.png}
...
\begin{equation}
  ...
  F_1(\tau) = \frac{2 \cdot P(\tau) \cdot R(\tau)}
                   {P(\tau) + R(\tau)}
  \label{eq:f1}
\end{equation}
...
\begin{table}[!t]
  ...
  \begin{tabular}{lcccccc}
    \toprule
    \textbf{Estratégia} & \textbf{$F_1$} & \textbf{MCC} & \textbf{Kappa}
                        & \textbf{Prec.} & \textbf{Recall} & \textbf{AUPRC} \\
    \midrule
    Sem tratamento          & 0,033 & 0,088 & 0,030 & 0,535 & 0,017 & 0,202 \\
    \textit{class\_weight}  & 0,026 & 0,086 & 0,024 & 0,632 & 0,013 & 0,191 \\
    SMOTE                   & 0,220 & 0,173 & 0,172 & 0,200 & 0,245 & 0,154 \\
    SMOTETomek              & \textbf{0,220} & 0,173 & 0,172 & 0,200 & 0,245 & 0,158 \\
    Undersampling           & 0,201 & \textbf{0,198} & 0,122
                            & 0,117 & \textbf{0,716} & \textbf{0,216} \\
    \textit{class\_weight} + $\tau^*$  & \textbf{0,262} & \textbf{0,217} & \textbf{0,214} & \textbf{0,231} & 0,302 & 0,191 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[!t]
  ...
  \caption{Comparação visual das estratégias RF por métrica ($F_1$, MCC,
           Kappa, Precision, Recall). SMOTETomek lidera em $F_1$ com
           threshold padrão; \textit{class\_weight}$+\tau^*$ lidera após ajuste.}
...
\begin{table}[!t]
  ...
  \begin{tabular}{lcccccc}
    \toprule
    \textbf{Configuração} & \textbf{$F_1$} & \textbf{MCC} & \textbf{Kappa}
                          & \textbf{Prec.} & \textbf{Recall} & $\tau$ \\
    \midrule
    RF Otimizado $\tau=0{,}5$ & 0,301 & 0,267 & 0,248 & 0,224 & 0,459 & 0,50 \\
    RF Otimizado $\tau^*$     & \textbf{0,329} & \textbf{0,299} & \textbf{0,297} & \textbf{0,378} & 0,291 & 0,65 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\begin{figure}[!t]
  ...
  \caption{Curva Precision-Recall e $F_1$ por threshold para o modelo
           \textit{class\_weight='balanced'}:
           $\tau^* = 0{,}65$ maximiza $F_1$ (AUPRC~$= 0{,}260$).}
...
\begin{table}[!t]
  ...
  \begin{tabular}{lcccccc}
    \toprule
    \textbf{Treino $\to$ Teste} & \textbf{$F_1$} & \textbf{MCC}
    & \textbf{Kappa} & \textbf{Prec.} & \textbf{Recall} & \textbf{AUPRC} \\
    \midrule
    221/074 $\to$ Intra   & 0,329 & 0,299 & 0,297 & 0,378 & 0,291 & 0,260 \\
    221/074 $\to$ 220/075 & \textbf{0,411} & \textbf{0,374} & \textbf{0,369}
                          & \textbf{0,486} & 0,356 & \textbf{0,467} \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\begin{table}[!t]
  ...
  \begin{tabular}{lrr}
    ...
    \midrule
    Predito Queimado     & TP = 16.252 & FP = 39.328 \\
    Predito Não queimado & FN = 59.905 & TN = 1.470.429 \\
    \midrule
    \textbf{Precision}  & \multicolumn{2}{c}{29,24\%} \\
    \textbf{Recall}     & \multicolumn{2}{c}{21,34\%} \\
    \textbf{$F_1$}      & \multicolumn{2}{c}{0,2468}  \\
    \textbf{\% pred.\ queimado} & \multicolumn{2}{c}{3,5\% (real: 4,8\%)} \\
    \bottomrule
  \end{tabular}
\end{table}
...
estratégias atingiam $F_1 = 1{,}000$ e Kappa~$= 1{,}000$. Após a remoção,
os valores caíram para intervalos realistas ($F_1 \in [0{,}03,\,0{,}41]$)
e a importância distribuiu-se uniformemente. Com $F_1$ como critério,
\textit{class\_weight=balanced} com ajuste de threshold ($\tau^* = 0{,}65$,
$F_1 = 0{,}262$) superou Undersampling ($F_1 = 0{,}201$) e
SMOTE/SMOTETomek ($F_1 \approx 0{,}220$), pois o threshold mais
conservador equilibra precisão e revocação ao invés de maximizar apenas
o recall. O resultado da validação cross-cena ($F_1 = 0{,}411$,
Precision~$= 0{,}486$) confirma que o modelo aprende relações espectrais
que transferem entre órbitas e entre Landsat~8 e~9, com melhor qualidade
geral (MCC~$= 0{,}374$, Kappa~$= 0{,}369$) em relação ao threshold padrão.
...
\begin{table}[!t]
  ...
  \begin{tabular}{llccccc}
    ...
    SOM$^{*}$     & 221/074            & —     & —     & —     & —     & 7  \\
    RF (blocos)   & 221/074            & 0,291 & 0,378 & 0,329 & 0,260 & 10 \\
    RF (cross)    & 220/075            & 0,356 & 0,486 & 0,411 & 0,467 & 10 \\
    \bottomrule
...
\begin{enumerate}
  ...
  \item O \textbf{RF com validação por blocos espaciais} forneceu a
        ...
        $F_1 = 0{,}411$ em generalização cross-cena, com distribuição
